...
def delete_done():
    ...
    user_trip = request.form.get('trip').strip()

    trip = Trip.query.filter_by(trip_name=user_trip, user_id=user_id).first()
    trip_id = trip.trip_id

    del_done = Todo_list.query.filter_by(trip_id=trip_id, todo=todo_done).first()
